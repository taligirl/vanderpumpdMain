...
export function AvatarCropModal({ src, onSave, onClose }: {
  src: string;
  onSave: (dataUrl: string) => void;
  onClose: () => void;
}) {
  const boxRef = React.useRef(null);
  const [cx, setCx] = React.useState(0.5);      // 0..1 center
  const [cy, setCy] = React.useState(0.5);      // 0..1 center
  const [scale, setScale] = React.useState(1);  // 1..3 zoom
  ...
  const doSave = async () => {
    const out = await cropToAspectDataURLAt(src, 256, 256, cx, cy, scale);
    onSave(out);
  };

  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Crop avatar">
      <div className="modal-panel">
        <h3 style={{ marginTop:0 }}>Crop avatar (drag to position, zoom)</h3>
        <div style={{ display:'grid', gap:10, marginTop:10 }}>
          <div
            ...
            style={{
              width:260, height:260, border:'1px solid #333', borderRadius:12, overflow:'hidden',
